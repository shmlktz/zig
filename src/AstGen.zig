...
fn asyncCallExpr(
    ...
    const call_inst = try callExprInner(gz, scope, node, call, .async_kw, Zir.Inst.AsyncCall);
...
fn callExpr(
    ...
    const call_inst = try callExprInner(gz, scope, node, call, modifier, Zir.Inst.Call);
